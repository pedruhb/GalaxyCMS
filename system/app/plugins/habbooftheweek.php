<?php
	...
	function userOfTheWeek()
	{
		...
		$getUserBadge = $dbh->prepare("SELECT badge_code,user_id,slot_id FROM users_badges WHERE user_id = :id AND slot_id = 1 LIMIT 1");
		...
		If($getUser->RowCount() > 0)
		{
			...
			echo '<div class="userNew" style="height: 110px;  background: url('.$config['avatarImageUrl'].'?figure='.filter($getUserData['look']).'&direction=2&head_direction=3&action=crr=667&gesture=sml);float: left;background-repeat: no-repeat;"></div>';
