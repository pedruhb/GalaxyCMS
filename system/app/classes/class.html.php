<?php
	...
	*/
	class Html 
	{
		...
		{
			global $dbh;
			$ipBan = $dbh->prepare("SELECT type,ban_expire FROM bans WHERE type = 'ip' && ip = :ip");
			$ipBan->bindParam(':ip', $ip);
			$ipBan->execute();
			if ($ipBan->RowCount() === 1)
			{
				$queryIp = $dbh->prepare("SELECT type,ban_expire FROM bans WHERE type = 'ip' && ip = :ip");
		...
		{
			echo '<div class="error e_top">'.$errorName.'</div>';
		}
		public static function errorSucces($succesMessage)
		{
			echo '<div class="sucesso" style="display: block;">'.$succesMessage.'</div>';
		}
