			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								<h4 class="mt-0 header-title">HK Logs</h4>
								<p class="text-muted mb-3">Veja os logs do hk, limitado aos últimos 5 mil.</p>
								<table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
									...
									<tbody>
										<?php
										$getArticles = $dbh->prepare("SELECT * FROM logs_hk order by id desc limit 5000");
										$getArticles->execute();
										while($result = $getArticles->fetch()){
											echo '<tr>
											<td>'.$result['id'].'</td>
											<td>'.GalaxyHK::GetUsernameFromId($result['user']).'</td>
											<td>'.date('d/m/y h:i', $result['data']).'</td>
											<td>'.utf8_decode($result['log']).'</td>
											</tr>';
										}
										?>	
									</tbody>
