<?php GalaxyHK::VerificaPermissao("bans");?>
<html lang="en">
			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								<h4 class="mt-0 header-title">Banimentos</h4>
								<p class="text-muted mb-3">Visualize e gerencie aqui os banimentos do hotel.</p>
								<?php
								if(isset($_GET['a'])){
									$id = $_GET['a'];
									$getArticles = $dbh->prepare("DELETE FROM bans WHERE id = :id LIMIT 1");
									$getArticles->bindParam(':id', $id);
									$getArticles->execute();
									GalaxyHK::Sucesso("Banimento ".$id." apagado com sucesso!");
									GalaxyHK::Log("Banimento ".$id." apagado com sucesso!", "Apagou o banimento ".$id);
								}
								?>
								<table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
									...
									<tbody>
										...
										while($result = $getArticles->fetch()){
											echo '<tr>
											<td>'.$result['id'].'</td>
											<td>'.$result['bantype'].'</td>
											<td>'.$result['value'].'</td>
											<td>'.utf8_decode($result['reason']).'</td>
											<td>'.date('d/m/y h:i', $result['added_date']).'</td>
											<td>'.date('d/m/y h:i', $result['expire']).'</td>
											<td>'.$result['added_by'].'</td>
											<td><a href="/adminpan/banimentos/apagar/'.$result['id'].'"><i class="fas fa-trash-alt text-danger font-16"></i></a></td>
											</tr>';
										}
										?>	
									</tbody>
