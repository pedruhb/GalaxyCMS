			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								<h4 class="mt-0 header-title">Notícias</h4>
								<p class="text-muted mb-3">Visualize e gerencie aqui as notícias postadas no hotel.</p>
								<?php
								if(isset($_GET['a'])){
									$id = $_GET['a'];
									$getArticles = $dbh->prepare("DELETE FROM cms_news WHERE id = :id LIMIT 1");
									$getArticles->bindParam(':id', $id);
									$getArticles->execute();
									GalaxyHK::Sucesso("Notícia ".$id." apagada com sucesso!");
									GalaxyHK::Log("Notícia ".$id." apagada com sucesso!", "Apagou a notícia ".$id);
								}
								?>
								<table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
									...
									<tbody>
										...
										while($result = $getArticles->fetch()){
											...
											<td>'.$result['id'].'</td>
											<td>'.$result['title'].'</td>
											<td>'.GalaxyHK::GetUsernameFromId($result['author']).'</td>
											<td>'.date('d/m/y h:i', $result['date']).'</td>
											<td><a href="/adminpan/editar-noticia/'.$result['id'].'" class="mr-2"><i class="fas fa-edit text-info font-16"></i></a> <a href="/adminpan/noticias/apagar/'.$result['id'].'"><i class="fas fa-trash-alt text-danger font-16"></i></a></td>
											</tr>';
										}
										?>	
									</tbody>
