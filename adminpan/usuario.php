			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								<h4 class="mt-0 header-title">Buscar usuário</h4>
								<p class="text-muted mb-3">Busque o usuário para editar a conta.</p>
								<?php
								if(isset($_POST['buscar'])){
									if(!empty($_POST['usuario'])){
										$getUsername = $dbh->prepare("SELECT id FROM users WHERE username = :a");
										$getUsername->bindValue(':a', $_POST['usuario']);
										$getUsername->execute();
										if($getUsername->rowCount() == 1)
											GalaxyHK::Sucesso("Usuário encontrado, clique <a href=\"/adminpan/editar-usuario/".$_POST['usuario']."\">aqui</a> para editar.");
										else
											GalaxyHK::Error("Usuário não encontrado no banco de dados.");
									} else {
										GalaxyHK::Error("Usuário em branco.");
									}
								}
								?>
								<form method="post">
									<div class="form-group">
										...
										<input type="text" class="form-control" placeholder="Usuário que deseja procurar, exemplo: PHB." name="usuario" required> 
									</div>
