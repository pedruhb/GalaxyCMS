			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								<h4 class="mt-0 header-title">Stafflogs</h4>
								<p class="text-muted mb-3">Veja os stafflogs do hotel, limitado aos últimos 5 mil.</p>
								<table class="table" id="datatable">
									<thead>
										<tr>
											<th class="wd-25p">Usuário</th>
											<th class="wd-25p">Quarto</th>
											<th class="wd-15p">Comando</th>
											<th class="wd-15p">Parâmetros</th>
											<th class="wd-20p">Data</th>
										</tr>
									</thead>
									<tbody>
										<?php
										$obtemLogs = $dbh->prepare("SELECT phbcommandlogs.timestamp, phbcommandlogs.params, users.username, rooms.name, commandlogs.command, phbcommandlogs.room_id FROM phbcommandlogs, users, rooms, commandlogs WHERE users.id = phbcommandlogs.user_id AND rooms.id = phbcommandlogs.room_id AND commandlogs.timestamp = phbcommandlogs.timestamp AND commandlogs.user_id = phbcommandlogs.user_id ORDER BY phbcommandlogs.timestamp DESC LIMIT 2500");
										$obtemLogs->execute();
										...
										?>
											<tr>
												...
												<td class="text-primary"><?= $log['command']; ?></td>
												<td class="text-primary"><?= $log['params']; ?></td>
